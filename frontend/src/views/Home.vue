    ...
    <div class="container mx-auto px-4 py-8 space-y-16">
      <section>
        ...
        <div v-if="loading.hotels" class="text-center p-10">
          <p class="text-gray-600">Đang tìm những khách sạn tốt nhất cho bạn...</p>
        </div>

        <div v-else-if="error.hotels" class="text-center p-10 bg-red-50 text-red-600 rounded-lg">
          <p>Rất tiếc, đã có lỗi xảy ra khi tải dữ liệu. Vui lòng thử lại sau.</p>
        </div>

        <div v-else-if="hotels.length > 0" class="relative group">
          <div ref="hotelScrollContainer" @scroll="() => handleScroll('hotels')"
            class="flex overflow-x-auto space-x-6 pb-4 scrollbar-hide snap-x snap-mandatory">
            <HotelCard v-for="hotel in hotels" :key="hotel.id" :hotel="hotel" class="snap-start" />
          </div>
          <button v-if="scrollState.hotels.canScrollLeft" @click="scroll('hotels', 'left')"
          ...
          <button v-if="scrollState.hotels.canScrollRight" @click="scroll('hotels', 'right')"
        ...
        <div v-else class="text-center p-10">
          <p class="text-gray-500">Không có gợi ý khách sạn nào vào lúc này.</p>
        </div>
      ...
      <section>
        ...
        <div class="relative group">
          <div ref="busScrollContainer" @scroll="() => handleScroll('buses')"
            class="flex overflow-x-auto space-x-6 pb-4 scrollbar-hide snap-x snap-mandatory">
            <BusCard v-for="bus in buses" :key="bus.id" :bus="bus" class="snap-start" />
          </div>
          <button v-if="scrollState.buses.canScrollLeft" @click="scroll('buses', 'left')"
            class="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-left"></i>
          </button>
          <button v-if="scrollState.buses.canScrollRight" @click="scroll('buses', 'right')"
            class="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-right"></i>
          </button>
        </div>
      ...
      <section>
        ...
        <div class="relative group">
          <div ref="flightScrollContainer" @scroll="() => handleScroll('flights')"
            class="flex overflow-x-auto space-x-6 pb-4 scrollbar-hide snap-x snap-mandatory">
            <FlightCard v-for="flight in flights" :key="flight.id" :flight="flight" class="snap-start" />
          </div>
          <button v-if="scrollState.flights.canScrollLeft" @click="scroll('flights', 'left')"
            class="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-left"></i>
          </button>
          <button v-if="scrollState.flights.canScrollRight" @click="scroll('flights', 'right')"
            class="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-right"></i>
          </button>
        </div>
        <div class="mt-8 text-center">
          <router-link :to="{ name: 'Plane' }"
            class="inline-block bg-blue-50 text-blue-700 font-semibold rounded-lg px-6 py-3 hover:bg-blue-100 transition-colors duration-200">
            Xem thêm Chuyến bay
          </router-link>
        </div>
      ...
      <section>
        ...
        <div class="relative group">
          <div ref="tourScrollContainer" @scroll="() => handleScroll('tours')"
            class="flex overflow-x-auto space-x-6 pb-4 scrollbar-hide snap-x snap-mandatory">
            <TourHomeCard v-for="tour in tours" :key="tour.id" :tour="tour" class="snap-start" />
          </div>
          <button v-if="scrollState.tours.canScrollLeft" @click="scroll('tours', 'left')"
            class="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-left"></i>
          </button>
          <button v-if="scrollState.tours.canScrollRight" @click="scroll('tours', 'right')"
            class="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-1/2 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-lg text-gray-700 hover:bg-gray-100 transition opacity-0 group-hover:opacity-100 z-10">
            <i class="fas fa-chevron-right"></i>
          </button>
        </div>
