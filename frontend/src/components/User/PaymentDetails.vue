<template>
    <section class="w-full max-w-3xl mx-auto space-y-10">
        <div>
            <h2 class="text-2xl font-semibold text-gray-800 mb-3">Phương thức thanh toán chính</h2>
        ...
        <div>
            <h3 class="text-xl font-semibold text-gray-800 mb-3">Danh sách thẻ đã lưu</h3>
                ...
                <div v-for="(card, index) in cards" :key="card.id"
                    ...
                    <div class="flex gap-3">
                        <button v-if="!card.primary" @click="setAsPrimary(index)"
                            class="text-sm text-blue-600 font-medium hover:underline">Đặt làm mặc định</button>
                        <button @click="removeCard(index)"
                            class="text-sm text-red-600 font-medium hover:underline">Xóa</button>
                    </div>
                </div>
            </div>
