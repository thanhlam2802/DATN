      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="col-span-2">
          ...
          <div class="flex flex-wrap gap-3">
            <div v-for="(img, idx) in images" :key="idx" class="relative w-28 h-28">
              <img :src="img.preview || img.url" class="w-full h-full object-cover rounded-xl border shadow" />
...
import { getAllAirlines, getAllAirports, getAllFlightCategories, updateAdminFlight, getFlightDetail, deleteAdminSeat, updateAdminFlightImages } from '@/api/flightApi'
...
async function submitUpdate() {
  ...
  // Xử lý cập nhật ảnh nếu có thay đổi
  const keepImageIds = images.value.filter(img => img.id).map(img => img.id)
  const newFiles = images.value.filter(img => img.file).map(img => img.file)
  if (keepImageIds.length > 0 || newFiles.length > 0) {
    const formData = new FormData()
    keepImageIds.forEach(id => formData.append('keepImageIds', id))
    newFiles.forEach(file => formData.append('files', file))
    const res = await updateAdminFlightImages(flight.value.id, formData)
    images.value = res.data.map(img => ({ id: img.id, url: img.imageUrl }))
  }
