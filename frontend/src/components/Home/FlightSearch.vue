    ...
    <div class="p-4 rounded-lg min-h-[250px]">
        <div class="flex flex-wrap items-center justify-between gap-4 mt-4 mb-4">
            <div class="flex items-center gap-2 p-1 bg-black/50 rounded-full">
                <button @click="$emit('update:tripType', 'round-trip')"
                    :class="tripType === 'round-trip' ? 'bg-white text-blue-600 shadow-md' : 'text-white/80 hover:bg-white/10'"
                    class="px-4 py-1.5 text-sm font-semibold rounded-full transition-all duration-300">
                    Khứ hồi
                </button>
                <button @click="$emit('update:tripType', 'one-way')"
                    :class="tripType === 'one-way' ? 'bg-white text-blue-600 shadow-md' : 'text-white/80 hover:bg-white/10'"
                    class="px-4 py-1.5 text-sm font-semibold rounded-full transition-all duration-300">
                    Một chiều
                </button>
                <button @click="$emit('update:tripType', 'many-city')"
                    :class="tripType === 'many-city' ? 'bg-white text-blue-600 shadow-md' : 'text-white/80 hover:bg-white/10'"
                    class="px-4 py-1.5 text-sm font-semibold rounded-full transition-all duration-300">
                    Nhiều thành phố
                </button>
            </div>
            ...
            <div class="w-full lg:w-5/12 flex items-center gap-2">
                <div class="relative w-full group">
                    ...
                    <div
                        ...
                        <i class="fas fa-plane-departure text-blue-500 text-xl px-4"></i>
                        <div class="py-2">
                            <p class="text-gray-900 font-bold text-lg">Hồ Chí Minh</p>
                            <p class="text-gray-500 text-sm">SGN - Sân bay quốc tế Tân Sơn Nhất</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative w-full group">
                    ...
                    <div
                        class="flex items-center bg-white rounded-lg border border-gray-300 overflow-hidden shadow-sm h-16">
                        <i class="fas fa-plane-arrival text-blue-500 text-xl px-4"></i>
                        <div class="py-2">
                            <p class="text-gray-900 font-bold text-lg">Bangkok</p>
                            <p class="text-gray-500 text-sm">BKK - Sân bay Suvarnabhumi</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full lg:w-4/12 flex items-center gap-2">
                <div class="relative w-full group">
                    ...
                    <div
                        ...
                        <div class="py-2">
                            <p class="text-gray-900 font-bold text-lg">18/06/2025</p>
                            <p class="text-gray-500 text-sm">Thứ Tư</p>
                        </div>
                    </div>
                </div>
                <div class="relative w-full group" :class="{ 'opacity-50 cursor-not-allowed': tripType === 'one-way' }">
                    <label for="return-flight"
                        class="absolute -top-2 left-3 px-1 bg-white text-gray-500 text-xs font-semibold rounded-md">Ngày
                        về</label>
                    <div
                        class="flex items-center bg-white rounded-lg border border-gray-300 overflow-hidden shadow-sm h-16">
                        ...
                        <div class="py-2">
                            <p class="text-gray-500 font-bold text-lg">
                                <span v-if="tripType === 'round-trip'">25/06/2025</span>
                                <span v-else>--/--/----</span>
                            </p>
                            <p class="text-gray-500 text-sm">
                                <span v-if="tripType === 'round-trip'">Thứ Tư</span>
                                <span v-else>Chọn ngày về</span>
                            </p>
                        </div>
                    </div>
                    <div v-if="tripType === 'one-way' || tripType === 'many-city'"
                        class="absolute inset-0 bg-gray-200/40 rounded-lg"></div>
                </div>
            </div>
