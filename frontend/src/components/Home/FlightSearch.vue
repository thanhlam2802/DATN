  ...
  <div class="p-4 rounded-lg min-h-[250px]">
    <div class="flex flex-wrap items-center justify-between gap-4 mt-4 mb-4">
      <div class="flex items-center gap-2 p-1 bg-black/50 rounded-full">
        <button
          @click="$emit('update:tripType', 'round-trip')"
          :class="
            tripType === 'round-trip'
        ...
        <button
          @click="$emit('update:tripType', 'one-way')"
          :class="
            tripType === 'one-way'
              ? 'bg-white text-blue-600 shadow-md'
              : 'text-white/80 hover:bg-white/10'
        ...
        <button
          @click="$emit('update:tripType', 'many-city')"
          :class="
            tripType === 'many-city'
              ? 'bg-white text-blue-600 shadow-md'
              : 'text-white/80 hover:bg-white/10'
      ...
      <div class="flex items-center gap-4">
        <div class="relative">
          <button
            class="flex items-center gap-2 text-white font-semibold text-sm"
          >
    ...
    <div class="flex flex-col lg:flex-row items-center gap-2 pt-8 pb-2">
      <div class="w-full lg:w-5/12 flex items-center gap-2">
        <div class="relative w-full group">
          <label
            for="from-flight"
            class="absolute -top-2 left-3 px-1 bg-white text-gray-500 text-xs font-semibold rounded-md"
            >Từ</label
          >
          <div
      ...
      <div class="w-full lg:w-4/12 flex items-center gap-2">
        ...
        <div
          ...
          :class="{ 'opacity-50 cursor-not-allowed': tripType === 'one-way' }"
        >
          <label
            for="return-flight"
            class="absolute -top-2 left-3 px-1 bg-white text-gray-500 text-xs font-semibold rounded-md"
          ...
          <div
            v-if="tripType === 'one-way' || tripType === 'many-city'"
            class="absolute inset-0 bg-gray-200/40 rounded-lg"
          ></div>
        </div>
      </div>
