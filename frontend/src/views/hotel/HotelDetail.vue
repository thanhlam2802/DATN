  ...
  <main v-if="hotel" class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8" style="padding-top: 0;">
      ...
      <section class="bg-white rounded-xl shadow p-6 mb-8">
        ...
        <div class="flex flex-wrap gap-4">
          <div v-for="filter in amenityFilters" :key="filter.id" class="flex items-center">
            ...
            <label :for="`filter-${filter.id}`" class="ml-2 block text-sm text-gray-900">
              {{ filter.label }}
            </label>
          </div>
        </div>
      </section>
      <div v-for="(room, idx) in filteredRoomTypes" :key="idx"
    ...
    <section class="w-full bg-[#d9f0ff] rounded-lg p-6 shadow-sm mb-8">
      <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
        <div class="flex-1">
          <h2 class="font-sans font-extrabold text-xl leading-6 mb-1 text-[#0a0a0a]">
        ...
        <div>
          <div
            ...
            <button aria-expanded="false" aria-haspopup="listbox"
              ...
              <svg class="w-3 h-3" fill="none" stroke="#0071c2" stroke-width="2" viewbox="0 0 24 24"
      ...
      <div aria-label="Accommodation suggestions" class="mt-6 flex gap-4 overflow-x-auto no-scrollbar pb-2" role="list">
        <article v-for="other in otherHotels" :key="other.id"
          class="bg-white rounded-lg shadow-sm min-w-[220px] max-w-[220px] flex-shrink-0 cursor-pointer hover:shadow-md transition"
          role="listitem" @click="router.push({ name: 'HotelDetail', params: { id: other.id } })">
          <div class="relative">
            <img :alt="other.alt || other.title" class="w-full h-[160px] object-cover rounded-t-lg" height="160"
              loading="lazy" :src="other.image" width="220" />
            <div
          ...
          <div class="p-3">
            <h3 class="font-sans font-bold text-sm leading-5 mb-0.5 text-[#0a0a0a]">
              {{ other.title }}
            </h3>
            ...
            <div class="flex items-center gap-1 mb-1">
              <div class="text-yellow-400 text-xs leading-none">
                <i class="fas fa-star" v-for="n in Math.round(other.rating)" :key="n"></i>
              </div>
              <span class="text-[#0071c2] text-xs font-semibold leading-none flex items-center gap-0.5">
                {{ other.rating }}
              </span>
            ...
            <div class="text-xs text-gray-400 line-through select-none" v-if="other.originalPrice">
          ...
          <div class="flex items-start gap-6">
            <div class="flex-shrink-0 w-40">
              <div class="flex items-center gap-4">
                <img :alt="review.name" class="w-12 h-12 rounded-full object-cover" :src="review.avatar" />
                <div>
                  <h3 class="text-base font-bold text-gray-900">{{ review.name }}</h3>
                  <p class="text-sm text-gray-500 mt-0.5">
                    <span class="hover:underline cursor-pointer" title="Đã đánh giá vào ngày này">{{ review.date
                    }}</span>
                  </p>
                </div>
              </div>
            ...
            <div class="flex-grow">
              ...
              <div class="mt-5 pt-4 border-t border-gray-100 flex flex-wrap items-center gap-3">
                <p class="text-sm font-medium text-gray-600">Đánh giá này có hữu ích không?
                </p>
                <div class="flex items-center gap-2">
                  <button
      ...
      <div class="mt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div class="flex items-center gap-2">
          ...
          <select id="reviews-per-page" v-model="reviewsPerPage"
            ...
            <option v-for="option in reviewsPerPageOptions" :key="option.value" :value="option.value">
              {{ option.text }}
            </option>
          </select>
        </div>
...
function onSearch() {
  ...
  router.push({ name: 'HotelListing', query: { ...searchParams.value, page: 1 } })
}
...
const minRoomPrice = computed(() => {
  const roomsToPrice = filteredRoomTypes.value.length > 0 ? filteredRoomTypes.value : roomTypes.value;
...
const reviewsList = ref([
  { name: 'John Smith', avatar: 'https://randomuser.me/api/portraits/men/32.jpg', rating: 4.5, date: 'Tháng 6, 2025', comment: 'Nơi tuyệt vời, dịch vụ xuất sắc và rất sạch sẽ. Rất khuyến khích!', },
  { name: 'Emily Johnson', avatar: 'https://randomuser.me/api/portraits/women/45.jpg', rating: 5, date: 'Tháng 5, 2025', comment: 'Vị trí hoàn hảo và chủ nhà siêu nhiệt tình!', },
  { name: 'Michael Brown', avatar: 'https://randomuser.me/api/portraits/men/28.jpg', rating: 4, date: 'Tháng 5, 2025', comment: 'Phòng đẹp, hơi ồn ào một chút nhưng có thể chấp nhận được.', },
  { name: 'Sarah Davis', avatar: 'https://randomuser.me/api/portraits/women/55.jpg', rating: 4.5, date: 'Tháng 4, 2025', comment: 'Giường rất thoải mái và tiện nghi tuyệt vời.', },
  { name: 'Tom Wilson', avatar: 'https://randomuser.me/api/portraits/men/52.jpg', rating: 3.5, date: 'Tháng 4, 2025', comment: 'Có thể cải thiện về độ sạch sẽ, nhưng nhìn chung là ổn.', },
  { name: 'Alex Martin', avatar: 'https://randomuser.me/api/portraits/men/45.jpg', rating: 4, date: 'Tháng 3, 2025', comment: 'Giá trị tốt so với số tiền bỏ ra và nhân viên thân thiện.', },
  { name: 'Jessica Lee', avatar: 'https://randomuser.me/api/portraits/women/65.jpg', rating: 5, date: 'Tháng 3, 2025', comment: 'Chắc chắn sẽ quay lại! Một trong những nơi tốt nhất tôi từng ở.', },
  { name: 'David Chen', avatar: 'https://randomuser.me/api/portraits/men/75.jpg', rating: 4, date: 'Tháng 2, 2025', comment: 'Mọi thứ đều tốt. Wifi hơi chậm vào buổi tối.', },
]);
...
const initializeRoomIndices = () => {
  roomTypes.value.forEach((room) => { roomImageIndex.value[room.id] = 0 });
};
...
const findHotel = () => {
  ...
  if (foundHotel) {
    hotel.value = {
      ...
      amenitiesList: [
        { name: 'Nhà bếp', icon: 'fas fa-utensils' }, { name: 'Bộ sơ cứu', icon: 'fas fa-first-aid' },
        { name: 'TV', icon: 'fas fa-tv' }, { name: 'Đồ uống miễn phí', icon: 'fas fa-wine-glass-alt' },
        { name: 'Điều hòa', icon: 'fas fa-snowflake' }, { name: 'Thuê xe đạp', icon: 'fas fa-bicycle' },
        { name: 'Wifi', icon: 'fas fa-wifi' }, { name: 'Hệ thống sưởi', icon: 'fas fa-thermometer-half' },
      ],
      additionalImages: [
    ...
    roomTypes.value = [
      { id: 1, label: 'Tiêu chuẩn', labelClass: 'bg-blue-100 text-blue-800', iconColor: 'text-blue-500', linkColor: 'text-blue-600', images: ['https://storage.googleapis.com/a1aa/image/344db20c-d038-4d9b-a473-fefd7c443548.jpg', 'https://storage.googleapis.com/a1aa/image/68c6fc18-96ef-43c2-3bae-b96409260526.jpg',], size: '17.0 m²', maxAdults: 2, maxChildren: 1, features: [{ name: 'Vòi tắm đứng', icon: 'fas fa-shower' }, { name: 'Tủ lạnh', icon: 'fas fa-snowflake' }, { name: 'Khu vực chờ', icon: 'fas fa-chair' }, { name: 'Máy lạnh', icon: 'fas fa-fan' },], type: 'Standard Room', rate: 'Standard Rate', bed: '1 giường cỡ king', tag: 'Vị trí tốt', tagClass: 'bg-blue-500', buttonClass: 'bg-blue-600 text-white hover:bg-blue-700', variants: [{ breakfast: 'Không bao gồm bữa sáng', hasBreakfast: false, cancellable: false, payAtHotel: false, originalPrice: '933.217', discountedPrice: '699.913', }, { breakfast: 'Bao gồm bữa sáng', hasBreakfast: true, cancellable: true, payAtHotel: true, originalPrice: '1.033.217', discountedPrice: '799.913', },], },
      { id: 2, label: 'Cao cấp', labelClass: 'bg-green-100 text-green-800', iconColor: 'text-green-500', linkColor: 'text-green-600', images: ['https://ik.imagekit.io/tvlk/generic-asset/dgXfoyh24ryQLRcGq00cIdKHRmotrWLNlvG-TxlcLxGkiDwaUSggleJNPRgIHCX6/hotel/asset/67778274-8c40a3ba78c6007e2e18af54a57143db.jpeg?_src=imagekit&tr=dpr-2,f-jpg,fo-auto,h-222,pr-true,q-40,w-320', 'https://storage.googleapis.com/a1aa/image/0216c56f-c55e-4854-998f-8a6098451dd3.jpg',], size: '20.0 m²', maxAdults: 2, maxChildren: 2, features: [{ name: 'Bồn tắm', icon: 'fas fa-bath' }, { name: 'Wifi miễn phí', icon: 'fas fa-wifi' }, { name: 'TV màn hình phẳng', icon: 'fas fa-tv' }, { name: 'Máy lạnh', icon: 'fas fa-fan' },], type: 'Deluxe Room', rate: 'Deluxe Rate', bed: '1 giường cỡ queen', tag: 'Phổ biến', tagClass: 'bg-green-500', buttonClass: 'bg-green-600 text-white hover:bg-green-700', variants: [{ breakfast: 'Không bao gồm bữa sáng', hasBreakfast: false, cancellable: false, payAtHotel: false, originalPrice: '1.200.000', discountedPrice: '950.000', }, { breakfast: 'Bao gồm bữa sáng', hasBreakfast: true, cancellable: true, payAtHotel: false, originalPrice: '1.400.000', discountedPrice: '1.100.000', },], },
      { id: 3, label: 'Thượng hạng', labelClass: 'bg-red-100 text-red-800', iconColor: 'text-red-500', linkColor: 'text-red-600', images: ['https://ik.imagekit.io/tvlk/generic-asset/dgXfoyh24ryQLRcGq00cIdKHRmotrWLNlvG-TxlcLxGkiDwaUSggleJNPRgIHCX6/hotel/asset/67778274-d22561c06b9bcec727af7a5fb1378627.jpeg?_src=imagekit&tr=dpr-2,c-at_max,f-jpg,fo-auto,h-222,pr-true,q-40,w-320', 'https://storage.googleapis.com/a1aa/image/27a56a3d-1d5e-4fa7-720b-0fe8b04c4867.jpg',], size: '22.0 m²', maxAdults: 3, maxChildren: 1, features: [{ name: 'Bar mini', icon: 'fas fa-wine-glass-alt' }, { name: 'Sofa', icon: 'fas fa-sofa' }, { name: 'TV màn hình phẳng', icon: 'fas fa-tv' }, { name: 'Máy lạnh', icon: 'fas fa-fan' },], type: 'Superior Room', rate: 'Superior Rate', bed: '1 giường cỡ king', tag: 'Ưu đãi đặc biệt', tagClass: 'bg-red-500', buttonClass: 'bg-red-600 text-white hover:bg-red-700', variants: [{ breakfast: 'Không bao gồm bữa sáng', hasBreakfast: false, cancellable: false, payAtHotel: false, originalPrice: '1.500.000', discountedPrice: '1.100.000', }, { breakfast: 'Bao gồm bữa sáng + minibar', hasBreakfast: true, cancellable: true, payAtHotel: true, originalPrice: '1.700.000', discountedPrice: '1.300.000', },], },
    ];
...
const prevRoomImage = (roomId) => {
  ...
  if (!room || !room.images) return;
  const count = room.images.length;
  roomImageIndex.value[roomId] = (roomImageIndex.value[roomId] - 1 + count) % count;
};
</script>
